...
const fetchStore = async () => {
  fetching.value = true
  try {
    const res = await http.get(`/store/${route.params.id}`)
    if (res.data.code === 1) {
      ...
      form.setValues({
        ...
        saleName: store.sale_name || store.saleName || '',
        salePosition: store.sale_position || store.salePosition || '',
        profileLogo: store.profile_logo || store.profileLogo || undefined,
        cover: store.cover || undefined,
      })
      
      if (store.profile_logo || store.profileLogo) {
        profileLogoPreview.value = store.profile_logo || store.profileLogo
      }
      if (store.cover) {
        coverPreview.value = store.cover
      }
    }
  } catch (error) {
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    const res = await http.put(`/store/${route.params.id}`, formData, {
      headers: {
    ...
    if (res.data.code === 1) {
      ...
      console.error('Error updating store:', res.data.message)
    }
  } catch (error) {
...
<template>
  <div class="container mx-auto py-6 max-w-4xl">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-3xl font-bold">Edit Store</h1>
    ...
    <div v-if="fetching" class="flex items-center justify-center py-12">
      <p class="text-muted-foreground">Loading store data...</p>
    </div>
    ...
    <form v-else @submit="onSubmit" class="space-y-6">
      ...
      <FormField name="profileLogo">
        <FormItem>
          ...
          <FormControl>
            <Upload 
              :imagePreview="profileLogoPreview"
