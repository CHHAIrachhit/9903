...
const fetchProduct = async () => {
  fetching.value = true
  try {
    const res = await http.get(`/product/${route.params.id}`)
    if (res.data.code === 1) {
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    const res = await http.put(`/product/${route.params.id}`, formData, {
      headers: {
...
<template>
  <div class="container mx-auto py-6 max-w-2xl">
    ...
    <div v-if="fetching" class="flex items-center justify-center py-12">
      <p class="text-muted-foreground">Loading product data...</p>
    </div>
